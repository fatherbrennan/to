...
import { getContent } from '$lib/utils/url';

let { url }: { url: URL; children?: Snippet } = $props();
...
<div class="flex flex-col size-full relative">
  {#if content.pageNumber !== null && content.pageIndex !== null && content.pages}
    <div
      class="flex flex-row absolute size-full justify-between z-10 pointer-events-none"
    >
      <a
        class={cls(
          "left-0 top-0 w-25",
  ...
  <div class="size-full flex flex-col gap-2 p-2 pt-footer">
    <div
      class="bg-[#d9e7d3] mx-auto my-auto p-8 w-9/12 h-8/12 flex flex-col gap-2 justify-center items-center rounded shadow-xs text-center text-sm sm:text-base"
    >
      {@render children?.()}
